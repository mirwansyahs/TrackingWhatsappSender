    ...
    <div class="card-body">
        <div class="tab-content" id="custom-tabs-four-tabContent">
            <div class="tab-pane fade show active" id="dataProduk" role="tabpanel" aria-labelledby="dataProduk-tab">
                <form method="POST" action="<?= base_url('admin/produk/' . $Produk->id ) ?>">
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" value="<?= $Produk->kelompok_barang ?>"
                                id="kelompok_barang" name="kelompok_barang" autocomplte="off">
                        </div>
                        <div class="form-group">
                            <label>Berat Produk (Gram)</label>
                            <input type="number" class="form-control" value="<?= $Produk->berat ?>" id="berat"
                                name="berat" autocomplete="off">
                        </div>
                    ...
                    <table class="table table-bordered table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td width="5%"><?=$no++?></td>
                                <td width="35%">
                                    <input type="hidden" name="produk_variasi_id"
                                        id="produk_variasi_id<?=$key->produk_variasi_id?>" class="form-control"
                                        value="<?=$key->produk_variasi_id?>">
                                    <input type="text" name="nama_variasi" id="nama_variasi<?=$key->produk_variasi_id?>"
                                        class="form-control" value="<?=$key->nama_variasi?>">
                                </td>
                                <td width="25%">
                                    <input type="text" name="harga" id="harga<?=$key->produk_variasi_id?>"
                                        class="form-control" value="<?=$key->harga?>">
                                </td>
                                <td width="15%" class="text-center">
                                    <a name="" id="" class="btn btn-primary btn-sm"
                                        onclick="btnSimpan('<?=$key->produk_variasi_id?>')" role="button"><i
                                            class="fa fa-edit"> Simpan</i></a>
                                    <a name="" id="" class="btn btn-danger btn-sm"
                                        href="<?=base_url()?>/admin/variasi/delete/<?=$key->produk_variasi_id?>"
                                        role="button"><i class="fa fa-trash"> Delete</i></a>
                                </td>
                            </tr>
                            <?php } ?>
                            <?php
...
<script>
    function btnSimpan(variasi_id = ''){
        ...
        $.post('<?=base_url()?>/admin/produk/createvariasi', 'nama_variasi='+nama_variasi+'&harga='+harga+'&kode_barang=<?=$Produk->kode_barang?>&produk_variasi_id='+produk_variasi_id, function(data){
            alert(data);
        });
    }
</script>
